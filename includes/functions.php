...
class Inventory {
	...
	private $supplierTable = 'supplier_table';
	private $purchaseTable = 'ims_purchase';
	private $orderTable = 'ims_order';
	private $dbConnect = false;
    // public function __construct(){
    //     if(!mysqli_connect("localhost", "root", "", "inventory")){ 
    //         $conn = new mysqli($this->host, $this->user, $this->password, $this->database);
    //         if($conn->connect_error){
    //             die("Error failed to connect to MySQL: " . $conn->connect_error);
    //         }else{
    //             mysqli_connect("localhost", "root", "", "inventory") = $conn;
    //         }
    //     }
    // }
	private function getData($sqlQuery) {
		$result = mysqli_query(mysqli_connect("localhost", "root", "", "inventory"), $sqlQuery);
		if(!$result){
	...
	public function saveCustomer() {		
		...
		mysqli_query(mysqli_connect("localhost", "root", "", "inventory"), $sqlInsert);
	...
	public function deleteCustomer(){
		$sqlQuery = "
			...
			WHERE id = '".$_POST['userid']."'";		
		mysqli_query(mysqli_connect("localhost", "root", "", "inventory"), $sqlQuery);		
	}
	// Category functions
	...
	public function categoryDropdownList(){		
		...
		$result = mysqli_query(mysqli_connect("localhost", "root", "", "inventory"), 'SELECT * from category_table');	
		$categoryHTML = '';
		while( $category = mysqli_fetch_assoc($result)) {
			$categoryHTML .= '<option value="'.$category["category_id"].'">'.$category["category_name"].'</option>';	
		}
		return $categoryHTML;
	}
	...
	public function updateBrand() {		
		if($_POST['id']) {	
			...
			mysqli_query(mysqli_connect("localhost", "root", "", "inventory"), $sqlUpdate);	
			echo 'Brand Update';
		}	
	}	
	...
	public function getProductDetails(){
		...
		// 	WHERE pid = '".$_POST["pid"]."'";
		$result = mysqli_query(mysqli_connect("localhost", "root", "", "inventory"), 'SELECT * from product_details WHERE product_id = "'.$_POST["product_id"].'"');			
		while( $product = mysqli_fetch_assoc($result)) {
	...
	public function updateProduct() {		
		if($_POST['product_id']) {	
			$sqlUpdate = "UPDATE product_table 
				SET category_id = '".$_POST['category_id']."', product_name='".$_POST['product_name']."', description='".$_POST['description']."', qty='".$_POST['qty']."', status='".$_POST['status']."', WHERE product_id = '".$_POST["product_id"]."'";			
			mysqli_query(mysqli_connect("localhost", "root", "", "inventory"), $sqlUpdate);	
			echo 'Product Update';
		}	
	}	
	...
	public function viewProductDetails(){
		$sqlQuery = "SELECT * FROM product_table as p
			
			INNER JOIN category_table as c ON c.category_id = p.category_id ";
	...
	public function manageProductDetails(){
		...
		$result = mysqli_query(mysqli_connect("localhost", "root", "", "inventory"), $sqlQuery);
		$productDetails = '<div class="table-responsive">
				<table class="table table-boredered datatable">';
		while( $product = mysqli_fetch_assoc($result) ) {
			$status = '';
			if($product['status'] == 'active') {
				$status = '<span class="label label-success">Active</span>';
			...
			<tr>
				...
				<td><button type="button" name="view" id="'.$product["product_id"].'" data-toggle="modal" data-target="#view-data" class="btn btn-info btn-sm rounded-0 view-data" title="View"><i class="bi bi-eye"></i></button>   <button type="button" name="update" id="'.$product["product_id"].'" class="btn btn-primary btn-sm rounded-0  edit-data" title="Update"><i class="bx bx-edit"></i></button>  <button type="button" name="delete" id="'.$product["product_id"].'" class="btn btn-danger btn-sm rounded-0  delete" data-status="'.$product["status"].'" title="Delete"><i class="bx bx-trash"></i></button></td>
			</tr>
			';
		}
		$productDetails .= '
	...
	public function productDropdownList(){	
		...
		$dropdownHTML = '';
		while( $product = mysqli_fetch_assoc($result) ) {	
			$dropdownHTML .= '<option value="'.$product["product_id"].'">'.$product["product-name"].'</option>';
		}
		return $dropdownHTML;
	}
